...
/// bytes before treating it as a silent MSE responder.
///
/// A MSE responder waits for our public key, so a peer that talks first is
/// either plaintext or broken. Per BEP 3 both ends send their BT handshake
/// right after connecting, so this detects plaintext peers that do so instead
/// of waiting out the full read/write timeout. A plaintext peer that stays
/// silent is not detected here: it costs this whole window, plus the rest of
/// `rwtimeout` waiting for a responder public key, before the caller redials
/// plaintext.
...
/// sha1("req2" || info_hash) — the MSE SKEY obfuscation key of a torrent.
fn req2_hash(info_hash: &[u8; 20]) -> [u8; 20] {
...
/// plaintext peer.
///
/// Per BEP 3 both ends speak first, so this only recognizes plaintext peers
/// that send their BT handshake promptly after connecting; one that stays
/// silent is not detected here and pays the sniff window plus the remaining
/// `rwtimeout` before the caller redials plaintext.
///
    ...
    W: AsyncWrite + Unpin,
{
    ...
    encrypted.extend_from_slice(
        &u16::try_from(pad_c.len())
            .expect("pad_c <= MAX_PAD")
            .to_be_bytes(),
    );
    encrypted.extend_from_slice(&pad_c);
    encrypted.extend_from_slice(
        &u16::try_from(BT_HANDSHAKE_LEN)
            .expect("handshake <= 65535")
            .to_be_bytes(),
    );
...
///
/// `info_hashes` is called lazily, and only when the initiator actually reaches
/// the SKEY step (i.e. it is never called for a plaintext peer), so the caller
/// can simply enumerate its known info hashes without paying for it on the
    ...
    F: Fn() -> Vec<[u8; 20]>,
{
    ...
    let info_hash = info_hashes()
        .into_iter()
        .find(|ih| req2_hash(ih) == skey_hash)
    ...
    response.extend_from_slice(
        &u16::try_from(pad_d.len())
            .expect("pad_d <= MAX_PAD")
            .to_be_bytes(),
    );
