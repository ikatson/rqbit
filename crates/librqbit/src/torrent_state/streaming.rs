...
use super::{ManagedTorrentHandle, ResolvedTorrent};
...
impl ManagedTorrent {
        ...
        F: FnOnce(&dyn TorrentStorage, &FileInfo) -> R,
    {
        self.with_state(|s| {
            ...
            let fi = resolved.file_infos.get(file_id).context("invalid file")?;
            Ok(f(files, fi))
        })
    }
    ...
    fn is_file_finished(&self, file_id: usize) -> bool {
        ...
        self.with_chunk_tracker(|ct| ct.is_file_finished(&resolved.file_infos[file_id]))
            .unwrap_or(false)
    }
