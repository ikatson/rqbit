...
impl Api {
            ...
            torrents
                .map(|(id, mgr)| {
                    let mut r = TorrentDetailsResponse {
                        ...
                        name: mgr
                            .with_resolved(|r| r.info.name.as_ref().map(|n| n.to_string()))
                            .ok()
                            .flatten(),
                        output_folder: mgr
                            .shared()
    ...
    pub fn api_torrent_details(&self, idx: TorrentIdOrHash) -> Result<TorrentDetailsResponse> {
        ...
        make_torrent_details(
            Some(handle.id()),
            &info_hash,
            handle.resolved.load().as_ref().map(|r| &r.info),
            only_files.as_deref(),
            output_folder,
        )
    }
    ...
    pub fn torrent_file_mime_type(
        ...
        let handle = self.mgr_handle(idx)?;
        handle.with_resolved(|r| torrent_file_mime_type(&r.info, file_idx))?
    }
...
fn make_torrent_details(
    ...
    Ok(TorrentDetailsResponse {
        ...
        name: info
            .map(|i| i.name.as_ref().map(|b| b.to_string()))
            .flatten(),
        files: Some(files),
        output_folder,
        stats: None,
