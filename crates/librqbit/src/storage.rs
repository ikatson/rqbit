...
use crate::{file_info::FileInfo, opened_file::OpenedFile, type_aliases::FileInfos};
...
pub trait TorrentStorage: Send + Sync {
    ...
    fn reopen(&self, file_id: usize, read_only: bool, file_info: &FileInfo) -> anyhow::Result<()>;

    fn take(&self) -> anyhow::Result<Box<dyn TorrentStorage>>;

    // To check if garbage collection is needed. Default is false, i.e. the storage is permanent.
    fn garbage_collection_needed(&self) -> bool;

    // Returns if the piece was removed
    fn garbage_collect_piece(&self, _id: ValidPieceIndex) -> bool;
}
...
impl TorrentStorage for FilesystemStorage {
    ...
    fn reopen(&self, file_id: usize, read_only: bool, file_info: &FileInfo) -> anyhow::Result<()> {
        self.opened_files[file_id].reopen(read_only, file_info)
    }
...
impl TorrentStorage for Box<dyn TorrentStorage> {
    ...
    fn reopen(&self, file_id: usize, read_only: bool, file_info: &FileInfo) -> anyhow::Result<()> {
        (**self).reopen(file_id, read_only, file_info)
    }
    ...
    fn garbage_collection_needed(&self) -> bool {
        (**self).garbage_collection_needed()
    }

    fn garbage_collect_piece(&self, id: ValidPieceIndex) -> bool {
        (**self).garbage_collect_piece(id)
    }
}
...
impl TorrentStorage for InMemoryGarbageCollectingStorage {
    ...
    fn garbage_collect_piece(&self, id: ValidPieceIndex) -> bool {
        info!(piece = id.get(), "gc storage: removing piece");
