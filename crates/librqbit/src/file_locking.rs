...
#[cfg(windows)]
    ...
    pub fn kill_processes_locking_path(path: &Path, recursive: bool) -> anyhow::Result<()> {
        ...
        // RM limit is high, but let's be safe and chunk if needed? 
        // For now, assume < 64k files.
        ...
        if res == ERROR_MORE_DATA {
            ...
            if n_proc_info > 0 {
                 tracing::warn!("Found {} processes locking files in {:?}. Shutting them down...", n_proc_info, path);
                 unsafe {
