...
pub struct PeerConnectionOptions {
    ...
    ///
    /// Defaulted on deserialization because this is the only non-`Option` field
    /// here: config/JSON written before MSE existed (desktop app, HTTP API
    /// clients) must keep loading instead of failing on the missing key.
    #[serde(default)]
