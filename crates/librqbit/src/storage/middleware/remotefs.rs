...
pub const REMOTEFS_PROTOCOLS: &[&str] = &[
    "http://",
    "https://",
    "sftp://",
    "ftp://",
...
pub struct RemoteFsStorageFactory {
    url: String,
}

impl RemoteFsStorageFactory {
    pub fn new(url: String) -> Self {
        Self { url }
    }
}

// impl<'a> StorageFactory for RemoteFsStorageFactory {
impl StorageFactory for RemoteFsStorageFactory {
    // type Storage = RemoteFsStorage<'a>;
    ...
    // ) -> anyhow::Result<RemoteFsStorage<'a>> {
    ) -> anyhow::Result<Self::Storage> {
        // log::info!("Creating RemoteFsStorage for URL: {}", self.url);
        ...
        if (protocol != "ftp") && (protocol != "ftps") {
            anyhow::bail!("RemoteFsStorageFactory only supports ftp:// and ftps:// URLs, got: {protocol}");
        }
...
pub struct RemoteFsStorage<'a> {
    fs: Arc<dyn RemoteFs>,
    shared: &'a ManagedTorrentShared,
    metadata: &'a TorrentMetadata,
}

// WONTFIX remotefs is not thread-safe, so we cannot implement Sync or Send for RemoteFsStorage
// the trait `Sync` is not implemented for `(dyn RemoteFs + 'static)`
// https://github.com/remotefs-rs/remotefs-rs/pull/21
impl<'a> TorrentStorage for RemoteFsStorage<'a> {
    fn pread_exact(&self, file_id: usize, offset: u64, buf: &mut [u8]) -> anyhow::Result<()> {
        ...
        log::info!("RemoteFsStorage pread_exact: file_id={}, offset={}, len={}, path={:?}", file_id, offset, buf.len(), relative_path);
    ...
    fn pwrite_all(&self, _file_id: usize, _offset: u64, _buf: &[u8]) -> anyhow::Result<()> {
        anyhow::bail!("RemoteFsStorage is read-only: pwrite_all is not implemented");
    }

    fn remove_file(&self, _file_id: usize, _filename: &std::path::Path) -> anyhow::Result<()> {
        anyhow::bail!("RemoteFsStorage is read-only: remove_file is not implemented");
    }

    fn remove_directory_if_empty(&self, _path: &std::path::Path) -> anyhow::Result<()> {
        anyhow::bail!("RemoteFsStorage is read-only: remove_directory_if_empty is not implemented");
    }

    fn ensure_file_length(&self, _file_id: usize, _length: u64) -> anyhow::Result<()> {
        anyhow::bail!("RemoteFsStorage is read-only: ensure_file_length is not implemented");
    }

    fn take(&self) -> anyhow::Result<Box<dyn TorrentStorage>> {
        anyhow::bail!("RemoteFsStorage take is not implemented")
    }
    ...
    fn init(
        ...
        // No-op for RemoteFsStorage, as it is read-only and uses references.
