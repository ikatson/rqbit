...
pub trait StorageFactory: Send + Sync + Any {
    ...
    fn create_and_init(
        ...
        let mut storage = self.create(info, resolved)?;
        storage.init(info, resolved)?;
        Ok(storage)
    }
    ...
    fn boxed(self) -> BoxStorageFactory {
        ...
        impl<SF: StorageFactory> StorageFactory for Wrapper<SF> {
            ...
            fn create(
                ...
                let s = self.sf.create(info, resolved)?;
                Ok(Box::new(s))
            }
